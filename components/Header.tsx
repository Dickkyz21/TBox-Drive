...
type SyncResult = { addedFiles: number; addedNotes: number } | { error: string };
...
export function Header({ fileCount }: { fileCount: number }) {
  ...
  async function handleSync() {
    ...
    try {
      ...
      if (!res.ok) setResult({ error: data.error || 'Sync gagal.' });
      else { setResult(data); router.refresh(); }
    } catch { setResult({ error: 'Koneksi gagal.' }); }
    finally {
      setSyncing(false);
    ...
    <header className="border-b border-base-700/60">
      <div className="flex items-center justify-between py-4 px-6 sm:px-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display font-semibold text-lg leading-tight">TeleDrive</h1>
            <div className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-ok-400 animate-pulse-dot" />
              <span className="text-xs text-ink-500">Terhubung · {fileCount} file</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {result && (
            <span className={`text-xs hidden sm:inline ${'error' in result ? 'text-danger-400' : 'text-ok-400'}`}>
              {'error' in result ? result.error
                : result.addedFiles + result.addedNotes > 0
                ? `+${result.addedFiles} file, +${result.addedNotes} catatan`
                : 'Sudah lengkap'}
            </span>
          )}
          <button onClick={handleSync} disabled={syncing}
            ...
            title="Sync dari Telegram">
            <svg viewBox="0 0 24 24" fill="none" className={`w-4 h-4 ${syncing ? 'animate-spin' : ''}`}>
              <path d="M4 4v5h5M20 20v-5h-5M19.4 9a8 8 0 0 0-14.8 0M4.6 15a8 8 0 0 0 14.8 0"
          ...
          return (
            <Link key={tab.href} href={tab.href}
              className={`px-3.5 py-2 text-sm font-medium rounded-t-lg border-b-2 transition-colors ${
                active ? 'text-tg-500 border-tg-500' : 'text-ink-500 border-transparent hover:text-ink-300'
              }`}>
              {tab.label}
            </Link>
          );
